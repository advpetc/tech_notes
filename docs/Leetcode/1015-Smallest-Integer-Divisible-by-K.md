...
By observation, we can find that the only case that will return -1 is when there is a repetend for the remainder as we grow our candidate number, so we can just create a set that will store all the previous remainders and see if the current remainder already exists or not. However, if we just arbitrarily increase our candidate number, it will result in overflow, so we need to apply the modulus trick:
...
Or intuitively, we can see that the remainder is always in the range from 0 to K. No matter how we increase our number, we just need to keep the remainder (% K result), since we are only interested in the remainder of the long number.

So we conclude that `r = (r * 10 + 1) % K `, where left-side r is our new r.

However, there is an even smarter way to reduce the space: 

```
Assume that N = 1 to N = K, if there isn't 111...11 % K == 0
There are at most K - 1 different remainders: 1, 2, .... K - 1.

So this is a pigeonhole problem:
There must be at least two identical remainders.
