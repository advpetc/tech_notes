...
To sort a linkedlist with constant space, we can use merge sort (for an array it will be $O(N)$ space).
...
3. use a temporary array to store the sorted array in the range of the current left - right
