...
Interval problems can usually be solved by using a sorting algorithm (or an inherently greedy algorithm). For this problem, we can sort the intervals by the start time. For two intervals `a` and `b`, where `a[0] <= b[0]`, we can know that if `a[1] >= b[1]`, the `b` interval will be covered by `a`. We can also know that `c` will also be covered if `a[1] > c[1]`. We don't need to know anything about the start time to check if the current interval can be covered, because the start time is always less than the current end time, and we are sure that the current interval's start time is less than the end time.
