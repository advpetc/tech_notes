...
Using this property, we don't really need to calculate the sum, but just need to check each digit's parity.

* Time: $O(n \times d)$ each number iterates through its digits to check the sum 
* Space: $O(1)$
