...
Time Complexity: if the tree is fully balanced (left height - right height abs value <= 1), then the tree will have at max $log(N)$ levels; for each node in the recursion tree, it will require $log(N)$ runs for the `while` loop, so the time complexity is $O(log(N) \times N)$

### Method 2: traverse right then left, then directly append
