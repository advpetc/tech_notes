...
Find the assignment such that the most workers can be matched (there is an obstacle in that **some bikes aren't reachable by certain cars**).
...
Starting from node 1, it matches with 6 (because 6 < 8), then node 2 matches with 5 (5 < 7): mark matches with red lines

![Screen Shot 2020-06-01 at 11.25.10 AM.png](resources/B2A81625BED77889675815994060603A.png)

When it comes to node 3, it can only match with 6, but node 6 is already matched with 1. However, since node 1 can also match with 8, node 1 will concede 6 and give it to node 3.
...
Since all workers can be freely assigned to all the bikes, and bikes can "choose" any of the workers, it's a complete bipartite graph. We should assume # of bikes != # of workers, or the ans is simply # of bikes or # of workers.
...
Use dfs to check if all the others can have the coloring.

**Now we have a different problem: find if the graph is bipartite when max = ?, and find the smallest max**
...
bool dfs(int u, int c, int limit)
{
    ...
    {
        if (w[i] <= limit) continue; // weight is less than current found max weight
        int j = e[i];
        if (color[j])
        {
            if (color[j] == c) return false; // in the same group
        }
        else if (!dfs(j, 3 - c, limit)) return false; // check alternative coloring => place j into another group
    }
...
bool check(int limit)
{
    ...
    for (int i = 1; i <= n; i ++ )
        if (color[i] == 0)
            if (!dfs(i, 1, limit)) // start by coloring the current node with 1, any that couldn't make it work will return false
                return false;
    return true;
}
