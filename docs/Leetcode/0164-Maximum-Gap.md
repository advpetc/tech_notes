...
Because we have n ranges, if we can know how big each range is, we can place each number into its correct range. For each range, we will keep a record of the maximum difference between a current number and the sorted number.
...
To determine which range each number should be placed into, we use a math trick:
...
`(n-1)(len-1) < Max - Min`, which yields 
...
public:
    ...
    int maximumGap(vector<int>& nums) {
        ...
        // but not across any two or more ranges
