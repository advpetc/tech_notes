...
When we see a combination problem, we can think of bit manipulation (0 represents not chosen and 1 represents chosen).
...
```
   Num             bit_rep           no_of_set_bit       a   b   c              hold_set
    ...
    2               0 1 0                 1              0   1   0              same case as the row above
...
public:
    ...
    string next() {
        ...
        string out;
        // step 2: check each character's location in the mask
        for(int i = 0; i < s.length(); i++) {
            if (mask & (1 << (s.length() - i - 1))) // chosen
                out += s[i];
        }
        // step 3: update mask to the next one
        mask--; 
        return out;
    }
    
    bool hasNext() {
        // step 1: try to find the next valid mask
        while(mask && __builtin_popcount(mask) != len) mask--;
        // step 2: see if it is found or not
        if (!mask)
            return false;
        return true;
    }
