...
Use an `unordered_map<int, int>` to keep track of the count of each element that appears.

- if k == 0, we just need to filter out all the elements that have the count greater than 1.
- else check the complement (curr element + k) if it exists in the map. We don't need to check element - k.
