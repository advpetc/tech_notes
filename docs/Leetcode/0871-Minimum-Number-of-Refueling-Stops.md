...
This problem can be solved using Dynamic Programming and a Greedy algorithm.

### Dynamic Programming
We could create a `dp[i]` array to represent how much fuel can be added at the `i` th station. To check if target miles can be reached at a given station, we just need to compare `dp[i]` and the target miles, because that means at the `i` th station we can go to `dp[i]` miles from our `dp[i]` fuel.
...
class Solution {
public:
    int minRefuelStops(int target, int startFuel/* 1 mile 1 fuel, so it means the initial target miles to the east can be reached */, 
...
public:
    int minRefuelStops(int target, int startFuel/* 1 mile 1 fuel, so it means the initial target miles to the east can be reached */, 
        ...
        for (auto& s : stations) {
            ...
            // cannot reach even using the maximum fuel
