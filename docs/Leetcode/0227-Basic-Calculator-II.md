...
We can use a stack to hold the previous value just like what we did for *basic calculator I* with parentheses. However, we don't need to use a stack for this problem since we don't have parentheses.
...
Another note: when we see '+' or '-' that means we can start calculating the result from the previous compound, and when we see '\*' or '/', we should start calculating the compound. This means the operation is always defined by the previous iteration, and in the current iteration we update the future state of the compound.
...
public:
    int calculate(string s) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // we only do an operation if we see an operator or if we're at the end
            if (currentChar == '+' || currentChar == '-' || 
                currentChar == '*' || currentChar == '/' || 
                i == length - 1) {
                if (sign == '+' || sign == '-') {
                    result += compound; // compound now has a sign from the previous operation
                    compound = (sign == '+') ? currentNumber : -currentNumber; // set the future sign
                } else if (sign == '*') {
