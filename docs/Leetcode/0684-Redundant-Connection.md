...
To quickly determine if there is a cycle given the edges, we can check if any two given nodes already share the same parent. With Union-Find and path compression, we can "almost" get it done in O(1).
...
-- as you can see, to determine if one node belongs to any root, we can run it in O(1)
