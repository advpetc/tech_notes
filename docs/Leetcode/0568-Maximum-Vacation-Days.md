...
From the days matrix, we can only move from left to right (except for day 1, in which we can move up and down) **or** left to down right by a certain amount of units. Now the problem becomes finding the highest weight path from top left to down right (with restrictions of the `flights` matrix and direction).

Let's discuss more about the moving direction. In the `days` matrix, the row represents the city number, and the column represents the week number. We can fly from any city to another one if there is a flight from the `flights` matrix. Or:
...
We need something to hold the current maximum, but we also know a local maximum cannot represent the global optimum, so we use dynamic programming. We use `dp[i][j]` to represent, on city i, week j, what the maximum vacation to take is. As we make progress to another week, we basically just try out all the previous maximum values with the current choice, and see if we should make the current decision.
...
Since I don't have premium for LeetCode, this code is copied and pasted from [Grandyang's blog](https://www.cnblogs.com/grandyang/p/6919389.html)
        ...
        for (int j = k - 1; j >= 0; --j) { // week j
            for (int i = 0; i < n; ++i) { // city 1
                ...
                for (int p = 0; p < n; ++p) {// city 2
                    if ((i == p || flights[i][p]) // city 1 == city 2 or there is a flight
                        && j < k - 1) { // and it's not the last week
                        dp[i][j] = max(dp[i][j], dp[p][j + 1] // since we progress downward, we populate week j + 1 before week j
                                       + days[i][j]);
                    }
                    if (j == 0 // we only update res when we reach starting city 0
                        && (i == 0 || flights[0][i])) // either the current city is already city 0 or there is a flight from the current city to city 0
                      res = max(res, dp[i][0]);
                }
            }
        }
