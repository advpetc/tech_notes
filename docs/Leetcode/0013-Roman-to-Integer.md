...
Notice that only 4, 40, and 400 have the roman character reversed (the smaller mapping is in front of the larger mapping), so we only need to handle the special case for that.
